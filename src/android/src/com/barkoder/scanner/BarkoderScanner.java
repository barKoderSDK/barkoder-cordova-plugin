...
//    this.cordova.getActivity().runOnUiThread(() -> { ; });
    ...
    private void setMulticodeCachingEnabled(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { BarkoderConfig.SetMulticodeCachingEnabled(multicodeCachingEnabled); });
    ...
    private void setMulticodeCachingDuration(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { BarkoderConfig.SetMulticodeCachingDuration(multicodeCachingDuration); });
    ...
    private void setMaximumResultsCount(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.config.getDecoderConfig().maximumResultsCount = maximumResultsCount; });
    ...
    private void setBarcodeThumbnailOnResultEnabled(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.config.setThumbnailOnResultEnabled(enabled); });
    ...
    private void setDuplicatesDelayMs(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.config.getDecoderConfig().duplicatesDelayMs = duplicatesDelayMs; });
    ...
    private void setThresholdBetweenDuplicatesScans(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.config.setThresholdBetweenDuplicatesScans(thresholdBetweenDuplicatesScans); });
    ...
    private void setUpcEanDeblurEnabled(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.config.getDecoderConfig().upcEanDeblur = enabled; });
    ...
    private void setMisshaped1DEnabled(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.config.getDecoderConfig().enableMisshaped1D = enabled; });
    ...
    private void setEnableVINRestrictions(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.config.getDecoderConfig().enableVINRestrictions = enableVINRestrictions; });
    ...
    private void isFlashAvailable(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.isFlashAvailable(value -> callbackContext.success(String.valueOf(value))); });
    }

    private void isCloseSessionOnResultEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isCloseSessionOnResultEnabled())); });
    }

    private void isImageResultEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isImageResultEnabled())); });
    }

    private void isLocationInImageResultEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isLocationInImageResultEnabled())); });
    }

    private void isLocationInPreviewEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isLocationInPreviewEnabled())); });
    }

    private void isPinchToZoomEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isPinchToZoomEnabled())); });
    }

    private void isRegionOfInterestVisible(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isRegionOfInterestVisible())); });
    }

    private void isBeepOnSuccessEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isBeepOnSuccessEnabled())); });
    }

    private void isVibrateOnSuccessEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.isVibrateOnSuccessEnabled())); });
    }

    private void getVersion(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(Barkoder.GetVersion()); });
    }
    ...
    private void getMaxZoomFactor(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { barkoderView.getMaxZoomFactor(value -> callbackContext.success(String.valueOf(value))); });
    }

    private void getLocationLineWidth(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.getLocationLineWidth())); });
    }

    private void getRoiLineWidth(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.getRoiLineWidth())); });
    }
    ...
    private void getMsiChecksumType(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().Msi.checksumType.ordinal()); });
    }

    private void getCode39ChecksumType(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().Code39.checksumType.ordinal()); });
    }

    private void getCode11ChecksumType(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().Code11.checksumType.ordinal()); });
    }

    private void getEncodingCharacterSet(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().encodingCharacterSet); });
    }

    private void getDecodingSpeed(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().decodingSpeed.ordinal()); });
    }

    private void getFormattingType(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().formattingType.ordinal()); });
    }

    private void getThreadsLimit(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(BarkoderConfig.GetThreadsLimit()); });
    }

    private void getMaximumResultsCount(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().maximumResultsCount); });
    }

    private void getDuplicatesDelayMs(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getDecoderConfig().duplicatesDelayMs); });
    }
    ...
    private void getMulticodeCachingEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(BarkoderConfig.IsMulticodeCachingEnabled())); });
    }

    private void getMulticodeCachingDuration(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(BarkoderConfig.GetMulticodeCachingDuration()); });
    }

    private void isUpcEanDeblurEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.getDecoderConfig().upcEanDeblur)); });
    }

    private void isMisshaped1DEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.getDecoderConfig().enableMisshaped1D)); });
    }

    private void isBarcodeThumbnailOnResultEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.getThumbnailOnResulEnabled())); });
    }

    private void getThresholdBetweenDuplicatesScans(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(barkoderView.config.getThresholdBetweenDuplicatesScans()); });
    }

    private void isVINRestrictionsEnabled(CallbackContext callbackContext) {
        this.cordova.getActivity().runOnUiThread(() -> { callbackContext.success(String.valueOf(barkoderView.config.getDecoderConfig().enableVINRestrictions)); });
    }
